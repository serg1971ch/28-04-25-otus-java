...
import ru.otus.minioBot.exceptions.EmptyMessageExceprion;
...
public class MessageParsingService {
    ...
    public Notification parseRemarkMessage(String userInput, Long chatId) throws IncorrectMessageException {
        ...
        if (matcher.matches()) {
            ...
            if (status.equals("да")) {
                statusComplete = TaskComplete.COMPLETE;
            } else {
                statusComplete = TaskComplete.UNCOMPLETED;
            }
            return new Notification(chatId, position, description, statusComplete, NotificationStatus.UNSENT, LocalDateTime.now());
        } else {
            logger.error("Парсинг не прошел. Input: '{}'. Описание на входе: {}", userInput, cleanedInput);
